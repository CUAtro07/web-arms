    ...
    if ($activityImage['error'] == 0) {
        ...
        if (in_array($image_file_type, ['jpg', 'jpeg', 'png', 'gif'])) {
            if (move_uploaded_file($activityImage['tmp_name'], $target_file)) {
                ...
                if ($stmt->execute()) {
                    // Return a response with a success message and a redirect to farmerportal.html
...
// Assuming you're using Fetch to submit the form data
document.getElementById("activityForm").addEventListener("submit", function(event) {
    event.preventDefault(); // Prevent default form submission

    var formData = new FormData(this); // Get form data

    fetch('upload_activity.php', {
        method: 'POST',
        body: formData
    })
    ...
    .then(data => {
        if (data.message) {
            ...
            if (data.redirect) {
                // Redirect to farmerportal.html if the activity is uploaded successfully
